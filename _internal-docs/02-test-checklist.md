...
* [x] Free recipe search by dish name, no photo/personal source/`recipeSourceProvider` required: "recipe for X" and "how do I/to make X" generate a generic home-style recipe via `generateRecipe({..., homeStyle: true})`; possessive phrasing ("my mom's recipe for X") still prefers a configured personal source over the generic fallback; non-matching text falls through to the ordinary photo prompt unchanged; graceful failure message (not a crash) when generation itself errors
