...
export const CreateBookingInputSchema = z.object({
  ...
  summary: z.string(),
  /**
   * The human-shareable identifier the chatter uses to look up, reschedule,
   * or (implicitly) prove ownership of this booking later — generated by
   * the agent, not the provider. Providers must index bookings by this
   * (e.g. Google Calendar's `extendedProperties.private`), not by parsing
   * `summary` text.
   */
...
export const FindBookingByAppointmentIdInputSchema = z.object({
  calendarId: z.string(),
  appointmentId: z.string(),
});
export type FindBookingByAppointmentIdInput = z.infer<typeof FindBookingByAppointmentIdInputSchema>;
...
export interface CalendarProvider {
  ...
  /** `null` if no booking with that appointment id exists on this calendar. */
